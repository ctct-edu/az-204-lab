...
6. 次のコマンドを実行して、キー コンテナーの名前を取得し、その名前を記録します。演習の後半で必要になります。
...
7. 次のコマンドを実行して、Azure Key Vault リソースを作成します。この処理の実行には数分かかる場合があります。
...
2. 次のコマンドを実行してシークレットを取得し、シークレットが設定されていることを確認します。
   ...
   このコマンドは、いくつかの JSON を返します。最後の行には、パスワードがプレーンテキストで含まれています。
...
2. 次のコマンドを実行して、コンソール アプリを起動します。アプリには、アプリケーションのメニュー システムが表示されます。
...
3. この演習の冒頭でシークレットを作成しましたが、**2** を入力して取得して表示します。

4. **1** を入力し、シークレット名と値を入力して、新しいシークレットを作成します。

5. シークレットをもう一度リストして、新しい追加を表示します。
