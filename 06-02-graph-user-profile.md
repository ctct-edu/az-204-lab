...
3. [**+ 新しい登録**] を選択し、[**アプリケーションの登録]** ページが表示されたら、アプリケーションの登録情報を入力します。
   ...
   | 畑                                     | 価値                                                         |
   | -------------------------------------- | ------------------------------------------------------------ |
   | **名前**                               | `myGraphApplicationXXXXXXXX`                                 |
   | **サポートされているアカウントの種類** | **[シングル テナントのみ - cludslice**] を選択します。       |
...
1. 上のメニューで **[File] > [New File...]** を選択した後、 **.env** と入力してEnterします。その後に表示されるファイル参照ダイアログで **Create File** をクリックします。これでプロジェクト フォルダーに *.env* という名前のファイルを作成しました。
...
3. 登録済みアプリに対して初めて認証する場合は、サインインしてプロファイルを読み取るためのアプリの承認、およびアクセス権を付与したデータへのアクセスを維持するように求める **[アクセス許可要求(Permission requested)]** 通知が届きます。**[同意する(Accept)]** を選択します。
